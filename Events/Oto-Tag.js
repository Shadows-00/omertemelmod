...
module.exports = (oldUser, newUser) => {
  ...
  if(!user) return;
  const embed = new discord.MessageEmbed().setAuthor(user.displayName, user.user.avatarURL({dynamic: true})).setFooter("Glosha ❤️").setColor(client.randomColor()).setTimestamp();
