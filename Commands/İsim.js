...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  message.channel.send(new MessageEmbed().setColor(client.randomColor()).setTitle("Kayıt Sistemi").setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setFooter("Glosha ❤️").setTimestamp().setDescription(`${emoji("gif1")} Kayıt Olan Kullanıcı: ${uye}\n${emoji("gif1")} Verilen Roller: ${uye.roles.cache.filter(a => a.name !== "@everyone").map(x => x).join(', ')}\n${emoji("gif1")} Kayıt Eden: ${message.author}\n${emoji("gif1")} Güncellenen İsim: \`${yazilacakIsim}\``)).catch();
};
