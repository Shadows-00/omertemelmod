...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setFooter("Glosha ❤️").setColor(client.randomColor()).setTimestamp();
  ...
  if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor(client.randomColor()).setTimestamp().setFooter("Glosha ❤️").setTitle('Üye Kicklendi!').setDescription(`**Kickleyen Yetkili:** ${message.author} (${message.author.id})\n**Kicklenen Üye:** ${victim.user.tag} (${victim.user.id})\n**Sebep:** ${reason}`));
};
